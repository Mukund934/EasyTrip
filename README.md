...
- ✅ **1,626 assertions across three layers** – 849 API tests against a real PostgreSQL, 636 component tests, and 141 browser journeys including ones that really sign in through the Firebase Auth Emulator and drive client-rendered pages as that user. The API and component counts were measured at Sprint 8.47; the browser count at Sprint 8.46, which came back 141 passed with nothing flaky and nothing skipped — the second complete run in a row, after three sprints in which the environment could not start the dev server at all (see `ENV-001`). Reproduce with `cd backend && npm test`, `cd frontend && npm test`, `npm run test:e2e`
