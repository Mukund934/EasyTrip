...
- ✅ **1,545 assertions across three layers** – 797 API tests against a real PostgreSQL, 607 component tests, and 141 browser journeys including ones that really sign in through the Firebase Auth Emulator and drive client-rendered pages as that user. The API and component numbers were measured at Sprint 8.45 by running both suites; **the browser count is carried forward from Sprint 8.42**, the last sprint whose environment could start the dev server (see `ENV-001`). Reproduce with `cd backend && npm test`, `cd frontend && npm test`, `npm run test:e2e`
