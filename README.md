...
- ✅ **1,591 assertions across three layers** – 823 API tests against a real PostgreSQL, 627 component tests, and 141 browser journeys including ones that really sign in through the Firebase Auth Emulator and drive client-rendered pages as that user. The API and component counts were measured at Sprint 8.46; the browser count at Sprint 8.45, which came back 141 passed with nothing flaky and nothing skipped — the first complete run since Sprint 8.42 (see `ENV-001`). Reproduce with `cd backend && npm test`, `cd frontend && npm test`, `npm run test:e2e`
